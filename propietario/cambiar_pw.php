...
else
{
  include("conexion.php");    
       ...
       isset($_POST['password_nuevo']) && !empty($_POST['password_nuevo']))  
    {
        ...
        $seleccion="SELECT * FROM propietarios WHERE id_propietario='$id_propietario'";
        ...
        $R=mysql_fetch_array($resul);
             ...
             if($password_viejo==$R['password']) 
             {
               if ($password_nuevo!=$password_viejo) 
                {
                  $sql="UPDATE propietarios SET password='$password_nuevo' WHERE id_propietario='$id_propietario'";
